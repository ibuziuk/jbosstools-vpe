...
public class CordovaJsServlet extends HttpServlet {
	...
	private String cordovaJsFileLocation;

	public CordovaJsServlet(String cordovaJsFile) {
		super();
		this.cordovaJsFileLocation = cordovaJsFile;
	}

	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if (cordovaJsFileLocation == null) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST); // 400
			return;
		} 
		
		File cordovaJsFile = new File(cordovaJsFileLocation);
		
        if (!cordovaJsFile.exists()) {
        	response.sendRedirect(DEFAULT_CORDOVA_JS); // Using cordova-3.1.0 engine by default
            return;
        }
